  ...
  const loadData = async () => {
    ...
    try {
      ...
      setIsOwnProfile(viewingOwnProfile);
        ...
        let feedPosts = [];
          if (viewingOwnProfile) {
            const { data: ownPosts } = await supabase.from('rposts').select(`id, title, text, created_at, user_id, zone_id, tag_id, image_url, image_urls, media_type, cta_type, cta_group_id, is_anonymous, moderation_status, is_deleted, posted_as_identity, spotify_url, user:rusers!user_id (username, emoji_icon, avatar_url, talent_showcase_id, business_showcase_id), zone:rzones!zone_id (name), tag:rtags!tag_id (name), poll_id, reactions:rreactions (reaction_type, device_id)`).eq('user_id', userData.id).eq('is_deleted', false).in('moderation_status', ['approved', 'held']).order('created_at', { ascending: false });
            
            const { data: friendPosts } = friendIds.length > 0 
              ? await supabase.from('rposts').select(`id, title, text, created_at, user_id, zone_id, tag_id, image_url, image_urls, media_type, cta_type, cta_group_id, is_anonymous, moderation_status, is_deleted, posted_as_identity, spotify_url, user:rusers!user_id (username, emoji_icon, avatar_url, talent_showcase_id, business_showcase_id), zone:rzones!zone_id (name), tag:rtags!tag_id (name), poll_id, reactions:rreactions (reaction_type, device_id)`).in('user_id', friendIds).eq('is_deleted', false).eq('moderation_status', 'approved').order('created_at', { ascending: false })
              : { data: [] };
